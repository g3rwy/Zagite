...
pub fn main() anyerror!void {
    ...
    while (! rl.WindowShouldClose())
    {
        ...
        // const grid_int = @floatToInt(c_int,grid_scale);
        ...
        if(scroll != 0){ // TODO probably need to clamp that or something
            grid_scale = @maximum(@minimum(grid_scale + scroll,100.0),2.0);
            grid_offset = @intToFloat(f32,@divFloor(GridToScreen(1,0).x - GridToScreen(0,0).x,2)); // calculated with each change of zoom
        }

        rl.BeginDrawing();
            rl.ClearBackground(rl.BLACK);
            const circ_screen = GridToScreen(circ_pos.x, circ_pos.y);
